...
class MC_Settings(Screen):
	...
	def checkNetworkStateFinished(self, result, retval, extra_args=None):
		...
		if result.find('bad address') != -1:
			self.session.openWithCallback(self.InstallPackageFailed, MessageBox, _("Your %s %s is not connected to the internet, please check your network settings and try again.") % (getOEMShowDisplayBrand(), getOEMShowDisplayModel()), type=MessageBox.TYPE_INFO, timeout=10, close_on_any_key=True)
