...
# now process existing timers
		...
		for timer in self.timerlist:
			if timer != self.newtimer:
				if timer.disabled or (not timer.conflict_detection and self.visible_conflict) or not timer.service_ref or '%3a//' in timer.service_ref.ref.toString() or timer.state == TimerEntry.StateEnded:
					continue
				if timer.repeated:
