		...
		for file in listdir('/etc/enigma2'):
			if file.endswith('.cache'):
				if file == 'networkbrowser.cache':
					...
					if fileExists(resolveFilename(SCOPE_GUISKIN, "networkbrowser/host.png")):
						hostpng = LoadPixmap(cached=True, path=resolveFilename(SCOPE_GUISKIN, "networkbrowser/host.png"))
					else:
