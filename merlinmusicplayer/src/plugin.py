...
# Lululla 20221010 as been fixed : nextitle, lyrics, major fix in skin and skin fhd.
...
from enigma import RT_VALIGN_CENTER, RT_HALIGN_LEFT, RT_HALIGN_RIGHT, RT_HALIGN_CENTER, gFont, eListbox, ePoint, eListboxPythonMultiContent, eTimer
from enigma import eServiceReference, getDesktop, ePicLoad, iPlayableService, iServiceInformation, eServiceCenter, getBestPlayableServiceReference
from enigma import eEPGCache, ePythonMessagePump
...
if DESKTOP_WIDTH == 1920:
    skin_path = resolveFilename(SCOPE_PLUGINS, "Extensions/{}/skin/fhd/".format('MerlinMusicPlayer'))
elif DESKTOP_WIDTH == 1280:
    skin_path = resolveFilename(SCOPE_PLUGINS, "Extensions/{}/skin/hd/".format('MerlinMusicPlayer'))
elif DESKTOP_WIDTH == 1024:
    skin_path = resolveFilename(SCOPE_PLUGINS, "Extensions/{}/skin/hd/".format('MerlinMusicPlayer'))
else:
...
class MerlinMusicPlayerScreen(Screen, InfoBarBase, InfoBarSeek, InfoBarNotifications):
    ...
    def getGoogleCover(self, artist, album, title, imagesize="&imgsz=medium"):
        if (artist == "" or artist == "n/a"):
            ...
            if (title == "" or title == "n/a"):
                ...
                url = "http://ajax.googleapis.com/ajax/services/search/images?v=1.0%s&q=%s+%s" % (imagesize, quote(title), quote(artist))
            ...
            url = "http://ajax.googleapis.com/ajax/services/search/images?v=1.0%s&q=%s+%s" % (imagesize, quote(album), quote(artist))
            sendUrlCommand(url, None, 10).addCallback(boundFunction(self.googleImageCallback, artist, album, title, imagesize)).addErrback(boundFunction(self.coverDownloadFailed, [], album, title))
    ...
    def coverDownload(self, urls, album, title):
        ...
        if (title != "" and title != "n/a"):
            filename = "%s_%s" % (sub(r'[^A-Za-z0-9_-]', r'', title), parts[-1])
        else:
            filename = "%s_%s" % (sub(r'[^A-Za-z0-9_-]', r'', album), parts[-1])
        if filename != self.currentGoogleCoverFile:
            self.currentGoogleCoverFile = filename
            filename = "%s%s" % (self.googleDownloadDir, filename)
            if os_path.exists(filename):
                ...
                downloadPage(ensure_binary(url), filename).addCallback(boundFunction(self.coverDownloadFinished, filename)).addErrback(boundFunction(self.coverDownloadFailed, urls, album, title))
    ...
    def getNextTitle(self):
        ...
        if self.iDreamMode or self.songList[index][0].PTS is not None:
            ...
            if self.songList[index][0].filename.lower().startswith("http://"):
                ...
                if audio:
                    if artist:
                        text = filename  # "%s - %s" % (title, artist)
                        # print('3 string ', type(text))
                    else:
                        text = filename  # title
                        # print('4 string ', type(text))
                else:
                    text = filename  # title
                    # print('5 string ', type(text))
                audio = None
        self.nextTitle = text
...
class MerlinMusicPlayerLyrics(Screen):
    ...
    def __init__(self, session, currentsong):
        ...
        self["actions"] = ActionMap(["WizardActions", "DirectionActions"],
                                    {
                                        "back": self.close,
                                        'left': self['lyric_text'].pageUp,
                                        'right': self['lyric_text'].pageDown,
                                        'up': self['lyric_text'].pageUp,
                                        'down': self['lyric_text'].pageDown,
                                        "upUp": self.pageUp,
                                        "leftUp": self.pageUp,
    ...
    def getLyricsFromID3Tag(self, tag):
        ...
        try:
            hdr = {"User-Agent": "Enigma2 - MerlinMusicPlayer Plugin"}
    ...
    def gotLyrics(self, xmlstring):
        ...
        if not lyrictext:
            ...
            artist = repr(artist)[2:-1]
            result = _("%s (%s)") % (title, artist)
            print('result = ', result)
...
class MerlinMediaPixmap(Pixmap):
    ...
    def updateCoverArt(self, path):
        ...
        for filename in self.coverFileNames:
            if fileExists("%s%s" % (path, filename)):
                new_coverArtFileName = "%s%s" % (path, filename)
...
#        self.list.append(self.googleimage)
        ...
        self["setupActions"] = ActionMap(["SetupActions",
                                          "ColorActions"], {"green": self.keySave,
...
class MerlinMusicPlayerFileList(Screen):
    ...
    def lcdupdate(self):
        ...
        text = sel[1][7] if sel[1][7] is not None else ""
        if sel[0][1] is True:
            text = "/%s" % text
